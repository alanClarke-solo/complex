...
// Generate a standard ULID
        long stTs = Instant.now().toEpochMilli();
...
// Extract timestamp from a ULID
        ...
        System.out.printf("Extracted timestamp: %s \n",timestamp);
        System.out.printf("Original timestamp: %s; difference %d \n", stTs, stTs-timestamp);
...
// For distributed services, use ULIDFactory with a unique node ID
        ...
        String ulidToNodeId = nodeFactory.generateMonotonic();
        int nodeId = ULIDFactory.extractNodeId(ulidToNodeId);
        System.out.printf("ULID: %s was generated by nodeId 42; extracted nodeId: %d", ulid, nodeId);
    }
}